...
<pre>
	...
	세션이란? 한 클라이언트(하나의 브라우져)가 어플리케이션을 사용하기 시작한 순간부터 사용 종료까지의 기간
	세션 생명주기
	...
	유지(tracking mode, ex) Cookie 형태) : C/S 사이에서 세션 아이디 공유 방법
		1. COOKIE : JSESSIONID(변경 가능)
		2. URL : 세션 파라미터(matrix variable)로 세션 유지
			<a href="sessionDesc.jsp;jsessionid=<%=session.getId() %>">URL 트래킹 모드 확인</a>
		3. SSL
	소멸(만료 정책) :
		1. logout(session invalidate) <%-- session.invalidate(); --%>
		2. tracking mode 에 따라 공유되던 session ID 를 분실
		3. 브라우저 종료(최근 브라우져는 종료시 세션 아이디를 분실시킨다)
